...
function getLoopData(id) {
  ...
  const rotations = rotate(chordData).map(getLoopId);
  const transitions = getTransitions(chordData);
  const type = getLoopType(transitions);

  const negative = getPrime(
    rotate(
      chordData.map((chord, _, list) => getNegative(chord, `${list[0].tonic}4`))
    ).map(getLoopId)
  );
  ...
  return {
    ...
    chords,
    rotations,
    // Currently only prime loops are generated
    prime: id,
    transitions,
    type,
    negative,
